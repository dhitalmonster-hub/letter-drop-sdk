...
describe('isValidContractName', () => {
  it('accepts standard names', () => {
    expect(isValidContractName('letter-drop')).toBe(true);
...
describe('parseContractId / formatContractId', () => {
  it('parses valid id', () => {
    const got = parseContractId('SP20Z3WPE6PVN1B8APDQNDH5BR1AJNMR25QPCSGFT.letter-drop');
    expect(got?.name).toBe('letter-drop');
  });
